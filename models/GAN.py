...
from tensorflow.python.keras.distribute.distributed_training_utils import _make_replicated_models_with_cloning
...
class GAN():
    ...
    def _build_discriminator(self):
        ...
        for i in range(self.n_layers_discriminator):
            ...
            x = Conv2D(
                filters = self.discriminator_conv_filters[i],
                kernel_size = self.discriminator_conv_kernel_size[i],
                strides = self.discriminator_conv_kernel_size[i],
                padding = 'same',
                name = 'discriminator_conv_' + str(i),
                kernel_initializer = self.weight_init
            )(x)
    ...
    def _build_adversarial(self):
        ...
        self.model.compile(optimizer=self.get_opti(self.generator_learning_rate), loss='binary_crossentropy', metrics=['accuracy'], experimental_run_tf_function=False)
    ...
    def train(self, x_train, batch_size, epochs, run_folder, print_every_n_batches=50, using_generator=False):
        ...
        for epoch in range(self.epoch, self.epoch + epochs):
            ...
            if epoch % print_every_n_batches == 0:
                self.sample_images(run_folder)
                self.model.save_weights(os.path.join(run_folder, 'weights/weights-%d.h5' % (epochs)))
                self.model.save_weights(os.path.join(run_folder, 'weights/weights.h5'))
                self.save_model(run_folder)
